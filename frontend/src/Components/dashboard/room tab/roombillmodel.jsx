...
function Bill({ checkedOutRoom, reservation, close }) {
  console.log(checkedOutRoom);
  ...
  const [rate, setRate] = useState(checkedOutRoom.price_per_night || 0);
  ...
  const { changeRoomStatus } = useRooms();

  const roomOrders = ordersData.filter(
    (o) =>
      o.locationType === "room" &&
      o.roomNumber === checkedOutRoom.room_number &&
      o.status === "delivered" &&
      o.customerName === reservation.guest?.full_name, //later put customerName here
  );
  ...
  const saveBill = async (roomId) => {
    try {
      const res = await privateAPI.post("/billing", {
        ...
        room_number: checkedOutRoom.room_number,
        customer_name: reservation.guest?.full_name || "Walk-in Guest",
        payment_method: paymentMethod,
        room_charges: roomCost,
        food_charges: ordersTotal,
        discount_amount: discountAmount,
        total_amount: finalAmount,
        bill_status: "PAID",
        room_ids: [checkedOutRoom.room_id], // many-to-many rooms relation
        guest_ids: [reservation.guest?.guest_id], // many-to-many guests relation
      });
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-gray-900 mb-3">
              Room {checkedOutRoom.room_number}
            </p>
            ...
            <div className="space-y-2 mb-3">
              ...
              {/* Per night rate */}
              <div className="flex items-center justify-between text-[15.5px]">
                <span className="text-gray-700">Per night rate</span>
                <div className="flex items-center gap-1">
                  <span className="text-gray-700">
                    Rs {checkedOutRoom.price_per_night}
                  </span>
                  <SquarePen
            ...
            <div className="border-t border-gray-200 pt-3 space-y-2">
              ...
              <div className="flex items-center justify-between border-t border-gray-200 pt-2">
                ...
                <span className="text-red-500">Rs {finalAmount}</span>
              </div>
            </div>
          </div>
          ...
          <div className="border border-purple-100 rounded-xl bg-purple-50">
            <div className="px-4 py-4 text-center">
              <p className="text-purple-700 text-sm">
                Amount to collect:{" "}
                <span className="text-purple-900">Rs {finalAmount}</span>
              </p>
            </div>
          </div>
            ...
            <div className="flex justify-center items-center gap-4">
              ...
              <button
                onClick={async () => {
                  await saveBill();
                  completeRoomOrders(
                    checkedOutRoom.room_number,
                    reservation.guest?.full_name,
                  );
                  await changeRoomStatus(checkedOutRoom.room_id, "Available");
