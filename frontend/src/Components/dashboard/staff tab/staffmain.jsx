...
function Staff() {
  ...
  const updateSalary = async () => {
    ...
    try {
      await privateAPI.put(`/staff/update-salary/${selectedStaff.user_id}`, {
  ...
  const handleDeleteStaff = async (id) => {
    try {
      await privateAPI.delete(`/staff/delete-staff/${id}`);
      // toast.success("Staff deleted successfully");
      fetchStaffs();
    } catch (error) {
      console.error(error);
      // toast.error("Couldn't delete staff. Try again");
    }
  };
  ...
  return (
    <div className="flex-1 bg-gray-50 p-8 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold">Staff Details</h1>
      ...
      {/* Add Staff Form */}
      {showAddStaffForm && (
        <AddStaffForm close={() => setShowAddStaffForm(false)} />
      )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          <div className="bg-white p-7 rounded-xl shadow-lg text-center w-100 flex flex-col items-center justify-center">
            <p className="font-medium text-lg mb-6">
              Do you want to remove {selectedStaff.full_name} from the system?
            </p>
            <div className="flex items-center justify-center gap-4">
              ...
              <button
                onClick={() => {
                  setShowPopUp(false);
                  handleDeleteStaff(selectedStaff.user_id);
                }}
